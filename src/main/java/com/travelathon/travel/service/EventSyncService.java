...
public class EventSyncService {
    ...
    private static final Logger logger =
            LoggerFactory.getLogger(EventSyncService.class);
    ...
    public int syncTicketmasterEvents() throws Exception {
        ...
        logger.info("Starting Ticketmaster sync...");
        ...
        if (embedded.isMissingNode() || embedded.isNull()) {
            ...
            logger.warn("No _embedded section found in Ticketmaster response");
        ...
        if (!eventsNode.isArray()) {
            ...
            logger.warn("No events array found in Ticketmaster response");
        ...
        for (JsonNode node : eventsNode) {
            ...
            String segmentName = node.path("classifications")
                    .path(0)
                    .path("segment")
                    .path("name")
                    .asText("");

            String genreName = node.path("classifications")
                    .path(0)
                    .path("genre")
                    .path("name")
                    .asText("");
            ...
            if (!venueNode.isArray() || venueNode.isEmpty())
                continue;
            ...
            if ("Sports".equalsIgnoreCase(segmentName)) {
            ...
            } else if ("Comedy".equalsIgnoreCase(genreName)
                    || "Comedy".equalsIgnoreCase(segmentName)) {
                ...
                logger.warn("Skipping unknown segment: {}", segmentName);
        ...
        logger.info("Ticketmaster events synced successfully. Count: {}", count);
